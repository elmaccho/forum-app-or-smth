...
<?php
// Sprawdź czy przesłano wszystkie wymagane dane
if (isset($_POST['newpostTitle']) && isset($_POST['newpostDesc']) && isset($_POST['postCategory'])) {
    ...
    $stmt = $conn->prepare("INSERT INTO posts (`tytul`, `autor_id`, `opis`, `kategoria`) VALUES (?, ?, ?, ?)");
    $stmt->bind_param("ssss", $newpostTitle, $userId, $newpostDesc, $postCategory);
    if ($stmt->execute()) {
